...
const outputPath = path.join(__dirname, '../src/data/flags.json');
...
try {
  ...
  const flagsData = svgFiles.map(filename => {
    const countryCode = path.basename(filename, '.svg');
    return {
      ...
      nodeId: '' // Will be empty since we're generating from existing files
    };
  });
  ...
  // Write to file
  fs.writeFileSync(outputPath, JSON.stringify(flagsData, null, 2));
  
  console.log(`✅ Generated flags.json with ${flagsData.length} entries`);
  console.log(`📁 Output: ${outputPath}`);
  
  // Show first 5 entries as sample
  console.log('\n📋 Sample entries:');
  flagsData.slice(0, 5).forEach(flag => {
    console.log(`   ${flag.countryCode}: ${flag.countryName}`);
  });
